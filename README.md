...
| 知识点体系 | 全局共享知识点库、学科内去重、层级维护、按名称解析入口 | Phase 1 完成 |
| 错题本 | 自动沉淀、艾宾浩斯复习调度、举一反三、用户反馈 | 骨架已完成 |
| 专注管理 | 番茄钟、防作弊 / 防沉迷、学习统计 | 骨架已完成 |
| 宠物激励 | AI 宠物对话、悬浮宠物聊天、学习事件反应、抚摸 / 玩耍、出门玩（限时游荡）、进化外观、智学币经济、Combo | 已完成 |
| 个人中心 | 头像、昵称、密码、学情资料、学习数据、会员展示 | 已完成 |
| 管理后台 | 独立 /admin 后台、用户管理、运营看板、内容管理 | 已完成 |
| AI 监控 | DeepSeek 余额与用量监控、低余额告警 | 已完成 |
...
- 宠物精灵图来自 [lencx/pet](https://github.com/lencx/pet)（MIT），用于 Web 端精灵动画与小程序端静态形象，许可证见 `web/public/pets/airi/LICENSE`。
...
| M1 用户 | 用户系统与冷启动问卷 | 已完成 |
| M2 规划 | AI 学习规划与动态调整 | 已完成 |
| M3 练习 | 出题、四重质检、错题复习调度 | 已完成 |
| M4 文件 | 文件出题 + RAG | 已完成 |
| M5 激励 | 专注、AI 宠物与宠物经济 | 已完成 |
| M6 演示 | 日历、提醒、课程聚合与部署 | 已完成 |
