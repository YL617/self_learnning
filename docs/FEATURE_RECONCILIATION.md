...
| 专家模板 + AI 微调生成破冰周计划 | 采纳 | 复用现有 `/plans/generate`；AI 不可用时明确报错 |
...
| AI 宠物对话与学习陪伴 | 采纳（V9.0 新增） | 宠物人格结合心情/饱食度/今日学习数据，DeepSeek 生成对话；失败明确报错 |
| 开源宠物形象 | 采纳 | 集成 MIT 开源 lencx/pet 精灵图，Web 端动画 + 小程序静态形象 |
